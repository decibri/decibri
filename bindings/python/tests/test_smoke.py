...
def test_package_imports() -> None:
    assert decibri.__version__ == "0.7.3"
...
def test_version_decibri_matches_rust_core() -> None:
    ...
    assert _decibri.MicrophoneBridge.version().decibri == "5.3.0"
...
def test_version_info_fields() -> None:
    ...
    info = _decibri.MicrophoneBridge.version()
    assert info.decibri == "5.3.0"
    assert info.audio_backend == "cpal 0.17"
    assert info.binding == "0.7.3"
...
def test_public_surface_count() -> None:
    """The public ``__all__`` enumerates the top-level surface (24 names).
    ...
    = 3 + 3 + 3 + 2 + 3 + 1 + 1 + 3 + 5 = 24.
    """
    assert len(decibri.__all__) == 24
