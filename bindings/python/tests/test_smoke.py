...
def test_package_imports() -> None:
    assert decibri.__version__ == "0.10.0"
...
def test_version_decibri_matches_rust_core() -> None:
    ...
    assert _decibri.MicrophoneBridge.version().decibri == "6.1.0"
...
def test_microphone_overrun_count_accessor() -> None:
    """overrun_count is readable and 0 on a fresh, not-yet-started stream."""
    mic = decibri.Microphone()
    try:
        assert isinstance(mic.overrun_count, int)
        assert mic.overrun_count == 0
    finally:
        mic.close()
...
def test_public_surface_count() -> None:
    """The public ``__all__`` enumerates the top-level surface (27 names).
    ...
    + 2 echo-cancellation types (Aec, AecMetrics)
    ...
    = 3 + 3 + 3 + 2 + 3 + 1 + 1 + 2 + 3 + 1 + 5 = 27.
    """
    assert len(decibri.__all__) == 27
