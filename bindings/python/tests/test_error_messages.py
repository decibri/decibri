...
def test_output_invalid_format_wrapper(format_value: str, expected_msg: str) -> None:
    """DecibriOutput rejects invalid format strings with the same message text as Decibri."""
    with pytest.raises(InvalidFormat) as exc_info:
        DecibriOutput(format=format_value)
...
def test_capture_read_before_start_message() -> None:
    """Decibri.read() before start() raises CaptureStreamClosed with canonical text."""
    d = Decibri()
...
def test_output_write_before_start_message() -> None:
    """DecibriOutput.write() before start() raises OutputStreamClosed with canonical text."""
    o = DecibriOutput()
...
def test_output_drain_before_start_message() -> None:
    """DecibriOutput.drain() before start() raises OutputStreamClosed with canonical text."""
    o = DecibriOutput()
    with pytest.raises(OutputStreamClosed) as exc_info:
        o.drain()
    assert str(exc_info.value) == "output is not running"
...
# VAD exceptions are unreachable from the public Decibri surface; they would
# fire only if a consumer constructed _decibri.DecibriBridge directly.
