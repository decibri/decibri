...
- `BlockSizeNotFrameAligned`, raised when a read asks for a block size that is not a whole number of frames. The bindings size every read as `frames_per_buffer` frames of the delivered count, so it is not reachable through `Microphone` or `AsyncMicrophone`; it is exported so the exception hierarchy stays complete against the core.
