...
- `AecMultichannelUnsupported`, raised at construction when `channels` is above 1 and `aec` names a model. The canceller reads one near-end channel: fed an interleaved multichannel capture it does not acquire its delay, returns the capture unchanged and reports no fault. Cancelling on one channel of a multichannel device is unaffected: `channels=1` with a `channel_map` naming that channel is accepted.
