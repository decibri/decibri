...
- `tests/test-ci.js` is the CI-safe test subset: hardware-free assertions across `assert`, `assertThrows`, and `assertRejects`, with the run printing its own pass and fail totals. Run it with `node tests/test-ci.js`.
