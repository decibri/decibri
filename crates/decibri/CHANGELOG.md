...
- An opt-in high-pass filter in the capture chain. `MicrophoneConfig` gains `highpass: Option<HighpassFilter>`, a `#[non_exhaustive]` cutoff selector re-exported as `decibri::HighpassFilter` with one variant today (`Hz80`, an 80 Hz second-order Butterworth high-pass). Naming a cutoff adds a same-length biquad stage to the capture chain after the denoise step, on the mono signal at the target rate, attenuating low-frequency rumble below the voice band while leaving the speech band essentially flat (the response is at the -3 dB point at the 80 Hz corner and near unity through the speech band). The filter is sample-in-sample-out and adds no latency (`latency_samples()` is zero), so it does not change the chain's transform latency or the VAD tap alignment. The cutoff set is `#[non_exhaustive]` so further cutoffs are a non-breaking widening, mirroring `DenoiseModel`. Default off (`None`), so the capture path stays byte-identical unless a consumer opts in.
...
- Multichannel capture is now downmixed to mono in the engine. A microphone opened with more than one channel delivers mono `AudioChunk`s (each frame averaged across its channels), and `MicrophoneStream::channels()` and `AudioChunk::channels` report the output channel count (1, mono). A single-channel device is unchanged and byte-identical. `next_chunk` / `try_next_chunk` are counted in OUTPUT samples (`frames_per_buffer * output_channels`). Breaking for Rust consumers that opened a multichannel device and read interleaved multichannel output.
