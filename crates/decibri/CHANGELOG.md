...
- `DecibriError::AecMultichannelUnsupported`, reported from `MicrophoneConfig::validate` when `aec` names a model and `channels` is above 1. Carries the count. The canceller reads one near-end channel: fed an interleaved multichannel capture it does not acquire its delay, returns the capture unchanged and reports no fault, and a discarded internal carry that is not a whole number of frames rotates the channel identities of everything after it, at any count above one. Neither outcome is visible in the delivered audio. Cancelling on one channel of a multichannel device is unaffected: `channels` of 1 with a `channel_map` naming that channel is accepted.
...
- Multichannel delivery on `File`: `AudioChunk::channels` carries the delivered count and every chunk is a whole number of interleaved frames, so no chunk boundary splits a frame. The detector feed and `File::analyze` score the frame average when the delivered audio carries more than one channel, the same collapse the live capture path's detector reads; at one channel the feed is unchanged byte for byte.
...
- `DecibriError::MultichannelNotSupported` is no longer returned by any operation. The variant, its `code()` and its message text are unchanged, so code matching either still compiles and still runs.
