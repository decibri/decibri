...
- `DecibriError::AecMultichannelUnsupported`, reported from `MicrophoneConfig::validate` when `aec` names a model and `channels` is above 1. Carries the count. The canceller reads one near-end channel: fed an interleaved multichannel capture it does not acquire its delay, returns the capture unchanged and reports no fault, and at a channel count that does not divide its internal framing it also cuts frames at the chunk boundaries. Neither outcome is visible in the delivered audio. Cancelling on one channel of a multichannel device is unaffected: `channels` of 1 with a `channel_map` naming that channel is accepted.
