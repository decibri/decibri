...
- `File`'s detector feed, drained by `File::vad_input`, holds two seconds of audio at the rate the feed carries: the detector rate with VAD configured, the target rate otherwise. The bound was sized from the larger of the two rates, so a feed that was never drained held more than two seconds at a target rate above the detector's with VAD configured, and at a target rate below it without. A consumer that drains the feed after every delivered chunk, as both bindings do, never reaches the bound and is unchanged.
