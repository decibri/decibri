...
- A `RangeError` from the constructor when `channels` is above 1 and `aec` names a model, with the message `echo cancellation runs on one delivered channel; channels is <n>. Set channels to 1 with a channel map to choose the device channel it cancels, or leave aec unset to capture more than one channel`. decibri runs the canceller on one near-end channel: fed an interleaved multichannel capture it does not acquire its delay, returns the capture unchanged and reports no fault. Cancelling on one channel of a multichannel device is unaffected: `channels: 1` with a `channelMap` naming that channel is accepted.
